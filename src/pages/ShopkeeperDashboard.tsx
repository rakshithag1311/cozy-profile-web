...
const ShopkeeperDashboard = () => {
  ...
  useEffect(() => {
    if (shop) {
      setOrders(getOrdersByShop(shop.id));
    }
                  ...
                  <div className="flex gap-2">
                    ...
                    {order.status === 'preparing' && (
                      <button
                        ...
                        className="flex-1 py-2 bg-success text-success-foreground rounded-xl font-semibold hover:opacity-90 transition-opacity"
                      >
                        Mark as Ready
                      </button>
                    )}
                    {order.status === 'ready' && (
                      <div className="flex-1 py-2 text-center text-success font-semibold">
                        ✓ Ready for Pickup
                      </div>
                    )}
                  </div>
